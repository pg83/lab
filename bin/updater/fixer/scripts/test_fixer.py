...
class FixerTests(unittest.TestCase):
    ...
    def test_prompt_asks_for_one_local_commit_without_publication(self):
        ...
        self.assertIn('finished only when that build exits zero', compact)
        self.assertIn('keep diagnosing and fixing it', compact)
        self.assertIn('Do not throw away a valid partial repair', compact)
        self.assertIn('create exactly one coherent local commit', compact)
        self.assertIn('concise factual commit message', compact)
        self.assertIn('Do not fetch, rebase, push, amend', compact)
        self.assertIn('supervisor exclusively owns remote publication', compact)
        ...
        self.assertNotIn('exits nonzero, do not commit', compact)
