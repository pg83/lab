...
# the ci.py S3_BUCKET=molot change. Drops old gorn/gorn/* task artifacts
# (gorn wrap's HEAD <uid>/result.json idempotency cache + recent task
# logs). Without this the new molot still
...
minio-client du minio/gorn/gorn/ minio/gorn/molot/
